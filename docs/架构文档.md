...
graph LR
  ...
  Key -->|local input| MC
  TIM2 -->|Flags| Main
  HAL -->|peripheral drivers| CAN & PWM & TIM1 & TIM2 & GPIO

  style CAN fill:#f9f,stroke:#333,stroke-width:1px
  style PWM fill:#ff9,stroke:#333,stroke-width:1px
  style MC fill:#9f9,stroke:#333,stroke-width:1px
```
...
> 注意：如果你将来需要把 mermaid 渲染为 PNG/SVG（用于 README 或 Wiki），可以使用 mermaid-cli 或 GitHub Actions 将其转换。
